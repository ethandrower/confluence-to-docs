...
/* Top-anchored: messages fill from the top, directly under the header. On a
   full thread this scrolls normally and useThreadScroll keeps the newest in
   view. Each item stays a real <li> flex-column so the <ol role="list"> keeps
   its list/listitem semantics. */
.mt-thread { list-style: none; margin: 0; padding: 20px 16px; display: flex; flex-direction: column; gap: 4px; flex: 1 1 auto; min-height: 0; overflow-y: auto; align-content: start; background: color-mix(in srgb, var(--muted) 45%, var(--background)); }
